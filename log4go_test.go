...
func TestFileLogWriter(t *testing.T) {
	...
	w := NewFileLogWriter(testLogFile, false, "day", "", 1)
	if w == nil {
...
func TestLogOutput(t *testing.T) {
	...
	l.AddFilter("file", FINEST, NewFileLogWriter(testLogFile, false, "day", "", 1).SetFormat("[%L] %M"))
	defer os.Remove(testLogFile)
...
func BenchmarkFileLog(b *testing.B) {
	sl := make(Logger)
	b.StopTimer()
	sl.AddFilter("file", INFO, NewFileLogWriter("benchlog.log", false, "day", "", 1))
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		sl.Log(WARNING, "here", "This is a log message")
	}
